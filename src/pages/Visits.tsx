                ...
                {selectedHistoryVisit && (
                    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/60 dark:bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-300">
                        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-300 border border-white dark:border-white/10">
                            <div className="p-8 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/5">
                                <div>
                                    <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">Detalle de Visita</h3>
                                    <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mt-1">ID: {selectedHistoryVisit.id.slice(-6)}</p>
                                </div>
                                <button onClick={() => setSelectedHistoryVisit(null)} className="text-slate-400 dark:text-slate-500 hover:text-slate-950 dark:hover:text-white transition-colors">
                                    <X size={24} />
                                </button>
                            </div>
                            ...
                            <div className="p-8 space-y-8 overflow-y-auto">
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <p className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{selectedHistoryVisit.clientName}</p>
                                    </div>
                                    <div className="flex gap-4">
                                        <div className="flex-1">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Fecha y Hora</p>
                                            <p className="font-bold text-slate-700 dark:text-slate-300">{selectedHistoryVisit.fecha} - {selectedHistoryVisit.hora}</p>
                                        </div>
                                        <div className="flex-1">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Gira</p>
                                            <p className="font-bold text-slate-700 dark:text-slate-300">{selectedHistoryVisit.gira}</p>
                                        </div>
                                    </div>
                                </div>

                                <div className="bg-slate-50 dark:bg-white/5 rounded-2xl p-6 border border-slate-100 dark:border-white/5">
                                    <p className="text-[10px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                        <Info size={12} /> Observaciones y Notas
                                    </p>
                                    <p className="text-slate-600 dark:text-slate-300 leading-relaxed text-sm font-medium whitespace-pre-wrap">
                                        {selectedHistoryVisit.notas || "Sin notas registradas."}
                                    </p>
                                </div>

                                {selectedHistoryVisit.sale && (
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Resumen de Venta</p>
                                            <span className="text-lg font-black text-emerald-600 dark:text-emerald-400">Total: Q {selectedHistoryVisit.sale.total.toFixed(2)}</span>
                                        </div>
                                        <div className="space-y-2">
                                            {selectedHistoryVisit.sale.items.map(item => (
                                                <div key={item.medicineId} className="flex justify-between items-center text-sm p-3 bg-slate-50 dark:bg-white/5 rounded-xl">
                                                    <div>
                                                        <span className="font-bold text-slate-700 dark:text-slate-300">{item.medicineName}</span>
                                                        <span className="text-xs text-slate-400 dark:text-slate-500 font-bold ml-2">x{item.cantidad}</span>
                                                    </div>
                                                    <span className="font-bold text-slate-900 dark:text-white">Q {item.subtotal.toFixed(2)}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                ...
                <div className="relative z-10 p-8 md:p-14 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="text-center md:text-left">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase tracking-[0.2em] mb-4">
                            ...
                            <span>{viewMode === 'today' ? `Hoy, ${today.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}` : 'Historial de Operaciones'}</span>
                        </div>
                        <h3 className="text-3xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tighter leading-none mb-4 italic">
                            {viewMode === 'today' ? (
                                ...
                                <>Archivo <span className="text-blue-600 dark:text-indigo-400">Histórico</span></>
                            )}
                        </h3>
                        <p className="text-slate-500 dark:text-slate-400 font-medium max-w-md opacity-80">
                            {viewMode === 'today'
                                ...
                                : 'Visualiza y analiza el registro completo de todas las intervenciones realizadas.'}
                        </p>
                    </div>
                    <div className="flex gap-4 flex-wrap justify-center">
                        <button
                            onClick={() => setViewMode(viewMode === 'today' ? 'history' : 'today')}
                            className={`px-6 py-4 rounded-[2rem] flex items-center gap-3 font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-lg ${viewMode === 'history' ? 'bg-slate-900 text-white shadow-slate-200' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-white/10 hover:bg-slate-50'}`}
                        >
                            {viewMode === 'today' ? <History size={18} /> : <Calendar size={18} />}
                            {viewMode === 'today' ? 'Ver Historial' : 'Ver Hoy'}
                        </button>
